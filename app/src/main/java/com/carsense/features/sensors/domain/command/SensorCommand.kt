...
abstract class SensorCommand : Command<SensorReading> {
    ...
    private fun cleanResponse(rawResponse: String): String {
        ...
        // Log for debugging
        println("SensorCommand: Cleaning response: $cleaned")

        return cleaned
    }

    /** Extract data bytes from a response */
    protected fun extractDataBytes(response: String): List<String> {
        println("SensorCommand: Extracting data bytes from: $response")
        ...
        if (response.contains(":")) {
            ...
            println("SensorCommand: Found header format with ':', data part: $dataPart")

            // Extract bytes after mode and PID
            if (dataPart.length >= 4) {
                val dataBytes = dataPart.substring(4).chunked(2)
                println("SensorCommand: Extracted data bytes: $dataBytes")
                return dataBytes
            }
        }
        // Check for format like "7E8 04 41 0C 12 34"
            ...
            if (modeIndex >= 0 && modeIndex + 2 < parts.size) {
                ...
                println(
                    "SensorCommand: Extracted data bytes from space-separated format: $dataBytes"
                )
                return dataBytes
            }
        }
        // Complex format like "7E804410C4233" (header + length + mode + pid + data)
            ...
            println(
                "SensorCommand: Looking for mode $expectedMode and PID $expectedPid in: $response"
            )
            ...
            if (modeIndex >= 0) {
                ...
                if (dataStart < response.length) {
                    ...
                    println(
                        "SensorCommand: Found mode+PID at index $modeIndex, extracted data: $dataBytes"
                    )
                    return dataBytes
                }
            } else {
                ...
                println("SensorCommand: Using fallback extraction, data: $dataBytes")
                return dataBytes
            }
        }
        ...
        println("SensorCommand: Fallback extraction result: $result")
        return result
    }
